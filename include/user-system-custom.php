...
if (isset($_GET['page']) && ($_GET['page'] == 'user-system' || $_GET['page'] == 'user-system-user' || $_GET['page'] == 'user-system-files' || $_GET['page'] == 'user-system-files-sent' || $_GET['page'] == 'user-system-messages' || $_GET['page'] == 'messages-users' || $_GET['page'] == 'files-users')) {
...
function usersystem_enqueue_styles(){
	...
	wp_enqueue_script('admin_js', plugin_dir_url(USER_PLUGIN_DIR) . 'user-system/assets/js/jquery-3.4.1.min.js', false);
	...
	wp_enqueue_style('dataTables-css', plugin_dir_url(USER_PLUGIN_DIR).'user-system/assets/css/jquery.dataTables.min.css');
	...
	wp_enqueue_script('dataTables-css', plugin_dir_url(USER_PLUGIN_DIR).'user-system/assets/js/jquery.dataTables.min.js', '', '', true);
	
	wp_enqueue_script('admin-custom-script', plugin_dir_url(USER_PLUGIN_DIR) . 'user-system/assets/js/custom-admin-script.js','','', true);

	wp_localize_script('admin-custom-script', 'adminpath', array(

		'ajaxurl' => admin_url('admin-ajax.php')

	));

	if( isset($_GET['page']) &&  ($_GET['page']== 'user-system-messages') ) {

		if( isset($_GET['user_id'] ) && !empty($_GET['user_id']) ) {
			
			$user_meta = get_userdata(trim(get_current_user_id()));
			$user_roles = $user_meta->roles[0]; // administrator
			// echo "<pre>";
			// var_dump($user_roles);
			// echo "</pre>";
			//creating php array 
			$data = array(
				...
				"receiver_id" => trim($_GET['user_id']),
				"user_role" => $user_roles
			);



			wp_enqueue_script('chat-admin-js', plugin_dir_url(USER_PLUGIN_DIR) . 'user-system/assets/js/chat-admin.js','','', true);

			// Passing above array to chat-js file
			wp_localize_script( "chat-admin-js", "blog", $data );
...
// Admin Socket on head
function admin_head_chat() { ?>
   <script src="https://cdnjs.cloudflare.com/ajax/libs/socket.io/1.4.5/socket.io.min.js"></script>
	<script type="text/javascript">var socket = io.connect("https://dev7.onlinetestingserver.com:9001/");</script>
<?php }
add_action( 'admin_head', 'admin_head_chat' );
...
function wpse_91693_register(){
	...
	add_menu_page('User System', // page title
	...
	// Universal user 
	add_menu_page('User System - User', // page title
	...
	add_menu_page('User System - Files', // page title
		'User System - Files', // menu title
		'manage_options', // capability
		'user-system-files', 'user_system_files', // callback function
		'dashicons-admin-site', // mene icons
		140
	);
	// Universal user files
	add_menu_page('User System -  Files Sent', // page title
	...
	add_menu_page('User System - Messages', // page title
	...
	add_menu_page(
		'Messages To Users', // page title
		'Messages To Users', // menu title
		'manage_options', // capability
		'messages-users', // menu slug
		'send_messages_users', //// callback function
		'dashicons-admin-site', // mene icons
		200
	);
	...
	add_menu_page(
		'Files To Users', // page title
		'Files To Users', // menu title
		'manage_options', // capability
		'files-users', // menu slug
		'send_files_users', //// callback function
		'dashicons-admin-site', // mene icons
		220
	);
...
function send_files_users(){

	$file = plugin_dir_path(__FILE__) . 'admin-templates/send-files-users.php';
	if (file_exists($file)) {

		require plugin_dir_path(__FILE__) . 'admin-templates/send-files-users.php';
	} else {
...
function send_messages_users(){

	$file = plugin_dir_path(__FILE__) . 'admin-templates/send-messages-users.php';
	if (file_exists($file)) {

		require plugin_dir_path(__FILE__) . 'admin-templates/send-messages-users.php';
	} else {
...
function usersystem_enqueue_front(){
	...
	wp_enqueue_style('usersystem_css', plugin_dir_url(USER_PLUGIN_DIR) . 'user-system/assets/css/custom.css');

	wp_enqueue_script('front-script', plugin_dir_url(USER_PLUGIN_DIR) . 'user-system/assets/js/jquery.dataTables.min.js', true);

	//wp_enqueue_script( 'stripe_lib_js_v3', 'https://js.stripe.com/v3/' ); 

	//wp_enqueue_script( 'stripe_lib_js_v2', 'https://js.stripe.com/v2/' ); 

	//wp_enqueue_script('stripe-script','https://js.stripe.com/v2/',array(),'1.1', true );

	/*wp_register_script('stripe-script', 'https://js.stripe.com/v3/', '', '', true);

	wp_enqueue_script('stripe-script');*/

	//wp_register_script( 'stripe-script', 'https://js.stripe.com/v3/', null, null, true );

	wp_enqueue_script('custom-script', plugin_dir_url(USER_PLUGIN_DIR) . 'user-system/assets/js/custom-script.js', array(), '1.0.0', true);

	wp_localize_script('custom-script', 'ajaxpath', array(
		'ajaxurl' => admin_url('admin-ajax.php')

	));

	if (is_page('user-system-messaging')) {
		...
		//creating php array 
			...
			wp_enqueue_script('chat-user-js', plugin_dir_url(USER_PLUGIN_DIR) . 'user-system/assets/js/chat-user.js','','', true);

			// Passing above array to chat-js file
			wp_localize_script( "chat-user-js", "blog", $data );
...
function user_head_chat() { ?>
   <script src="https://cdnjs.cloudflare.com/ajax/libs/socket.io/1.4.5/socket.io.min.js"></script>
	<script type="text/javascript">var socket = io.connect("https://dev7.onlinetestingserver.com:9001/");</script>
<?php }
add_action( 'wp_head', 'user_head_chat' );
